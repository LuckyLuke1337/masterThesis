...
def main(nJump, step, m, n, k, DATFILE, function):
    dimension = 10                                                      # Number of dimensions of the problem
    # -------------- Control Parameters of SOMA -------------------------------
    # nJump, step = 10, 0.3                                              # Assign values ​​to variables: step, PRT, PathLength
    ...
    fitness = CostFunction(pop.T)                                         # Evaluate the initial population
    ...
    while FEs < Max_FEs:                                                # Terminate when reaching Max_Migration / User can change to Max_FEs
        ...
        for j in range(n):                                              # Choose n individuals move toward the Leader
            ...
            flag, move = 0, 1
            while (flag == 0) and (move <= nJump):
                nstep = (nJump-move+1) * step
                ...
                new_cost = CostFunction(offspring.T)                      # Evaluate the offspring
        ...
        if Count > PopSize*50:
            if the_best_cost == best_cost_old:
                ...
                fit_temp = CostFunction(pop_temp.T)
                FEs = FEs + rat
...
if __name__ == "__main__":
    ...
    ap.add_argument('--nJump', dest='nJump', type=float, required=True, help='nJump')
    ...
    main(args.nJump, args.step, args.m, args.n, args.k, args.datfile, args.function)