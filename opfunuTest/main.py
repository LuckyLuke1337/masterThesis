...
def main(m, n, k, nJumps, DATFILE, function):
    ...
    Dim = 30
    functionID=function
    problem = getattr(opfunu.cec_based, functionID)(ndim=Dim)
    ...
    for i in pop:
        fit = numpy.append(fit, problem.evaluate(i))
    ...
    while FEs < Max_FEs:
        Mig = Mig + 1
        # ------------ Migrant selection: m -----------------------------------
        M = numpy.random.choice(range(NP),m,replace=True)
        ...
        for i in newpop:
            newfitpop.append(problem.evaluate(i))
